...
export function UploadImage({ onImageAdded, image }: {
    ...
    async function onFileSelect(e: any) {
        ...
        try {
            ...
            const response = await axios.get("http://localhost:3001/v1/user/getPresingedUrl", {
                headers: {
                    "Authorization": localStorage.getItem("token")
                }
            });
            const presignedUrl = response.data.preSigned;
            ...
            onImageAdded(`https://d2bsx6om6afntv.cloudfront.net/${response.data.fields["key"]}`);
        } catch(e) {
            console.log(e)
        }
        setUploading(false);
